...
export default function Login() {
  ...
  const handleSubmit = (e: any) => {
    e.preventDefault();
    handleEmailSignIn(email, password); // Call the email sign-in function
  };
  ...
  const handleEmailSignIn = async (email: string, password: string) => {
    try {
      ...
      // Call your login API to generate a session cookie
      ...
      if (response.ok) {
        ...
        setUser(userData);
        router.push("/student"); // Navigate to the protected route
      } else {
        console.error("Login API failed:", response.status);
      }
    } catch (error) {
  ...
  const handleGoogleSignIn = async () => {
    ...
    return (
      <>
        <div className="flex flex-col items-center justify-center p-0 mx-auto md:h-screen lg:py-0">
          <div className="w-full bg-white rounded-lg shadow dark:border md:mt-0 sm:max-w-md xl:p-0 dark:bg-gray-800 dark:border-gray-700">
            <div className="p-6 space-y-4 md:space-y-6 sm:p-8">
              ...
              <form className="space-y-4 md:space-y-6" onSubmit={handleSubmit}>
                ...
                <button
                  type="submit"
                  className="w-full rounded-lg border border-solid border-transparent transition-colors flex items-center justify-center bg-foreground text-background gap-2 hover:bg-[#383838] dark:hover:bg-[#ccc] text-sm sm:text-base h-10 sm:h-12 px-4 "
                >
                  Öğrenci Girişi
                </button>
