...
export default function Quiz() {
  ...
  useEffect(() => {
    if (!quizState) {
      ...
      setCorrectCount(statuses.filter((status) => status).length);
    }
  }, [quizState]);
      ...
      {!quizState && (
            ...
            <ul>
              {answerStatuses.map((status, index) => (
                <li key={index} className="m-1">
                  {status ? <Badge color="success">Doğru</Badge> : <Badge color="failure">Yanlış</Badge>}
                </li>
              ))}
            </ul>
          </div>
        ...
        <div className="flex justify-end items-end p-1 sm:mr-10">
              <h2 className="text-xl font-bold m-1">Toplam: </h2>
                <Badge color="success" className="m-2 w-20">{correctCount} Doğru</Badge> 
                <Badge color="failure" className="m-2 w-20">{9-correctCount} Yanlış</Badge> 
